...
--current overall rate is. The Regis column contained two different date formats in the raw file, so TRY_STRPTIME with two format 
--patterns was used to read it correctly.
    YEAR( 
        COALESCE( 
...
--Why this query: compares average transaction value for delayed vs. normal transactions to see whether higher-value deals are 
--more or less likely to be delayed. ProcedureValue is stored as text in the file, so TRY_CAST is used to safely convert it to a number.
SELECT 
...
--Why this query: looks at revenue exposure rather than transaction count, and filters out very low-volume projects (fewer than 30 
--transactions) so a single delayed sale in a tiny project doesn't distort the ranking. 
SELECT 
Project, 
